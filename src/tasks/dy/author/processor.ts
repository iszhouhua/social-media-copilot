...
export class Processor extends TaskProcessor<FormSchema> {
    ...
    async execute(signal: AbortSignal) {
        ...
        for (const url of urls) {
            ...
            const user: DataValue = await http.user.userProfileOther(url.id).then(res => res.user);
            if (needInteractionInfo) {
                user.awemes = await this.getLastAwemes(url.id, 10);
            }
    ...
    async getLastAwemes(
        ...
        const list: http.aweme.AwemeDetail[] = await http.aweme.awemePost({
            sec_user_id: userId,
            count: limit,
            cut_version: 1,
            max_cursor: 0
