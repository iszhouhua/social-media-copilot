...
export default class XhsProcessor extends TaskProcessor<z.infer<typeof formSchema>, WebV1Feed> {
    ...
    async execute() {
        ...
        for (let i = 0; i < postIds.length; i++) {
            ...
            const post = await this.request(webV1Feed, postId);
            this.data[postId] = post;
    ...
    async getFileInfos(): Promise<Array<FileInfo>> {
        ...
        for (const noteId of this.condition.postIds) {
            const feed: WebV1Feed = this.data[noteId];
