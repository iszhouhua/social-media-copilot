...
import { webV1UserPosted, webV1UserOtherinfo, type WebV1UserOtherinfo } from "@/services/xhs/user";
...
export default class XhsProcessor extends TaskProcessor<FormSchema, DataValue> {
    ...
    async getFileInfos(): Promise<Array<FileInfo>> {
        ...
        return [getExcelFileInfo(dataList, "博主数据导出")];
    }
    ...
    async getLastNotes(
        ...
        for (const note of result.notes) {
            const feedData = await this.request(webV1Feed, note.note_id);
