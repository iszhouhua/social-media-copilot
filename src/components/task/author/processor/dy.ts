import { FileInfo, getExcelFileInfo, TaskProcessor } from "@/components/task";
import type { AwemeDetail, Statistics } from "@/services/dy/aweme.d";
import { FormSchema } from "..";
import { awemePost } from "@/services/dy/aweme";
import { userProfileOther, UserInfo } from "@/services/dy/user";


type DataValue = UserInfo & {
    awemes?: Array<AwemeDetail>
}

export default class DyProcessor extends TaskProcessor<FormSchema, DataValue> {

    async execute() {
        const { authorIds, needInteractionInfo } = this.condition;
        this.actions.setTotal(authorIds.length);
        for (const authorId of authorIds) {
            const user: DataValue = await this.request(userProfileOther, authorId).then(res => res.user);
    ...
    async getFileInfos(): Promise<Array<FileInfo>> {
        ...
        for (const authorId of this.condition.authorIds) {
            ...
            if (this.condition.needInteractionInfo) {
                ...
                const median = (fieldName: keyof Statistics | "interaction_count") => {
                ...
                const average = (fieldName: keyof Statistics | "interaction_count") => {
        ...
        return [getExcelFileInfo(dataList, "达人数据导出")];
    }
    ...
    async getLastAwemes(
        ...
        const list: AwemeDetail[] = await this.request(awemePost, {
            sec_user_id: userId,
            count: limit,
            cut_version: 1,
            max_cursor: 0
